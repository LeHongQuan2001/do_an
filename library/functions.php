...
function getGiaoVienRecords(){
	...
	if($type == 'giaovien') {
		...
		$sql = "SELECT  * FROM tbl_users u WHERE type != 'admin' AND type !='giaovu'  ORDER BY u.id DESC LIMIT $start, $per_page";
	}
...
function generateClassPagination() {
	$per_page = 10;
	$sql 	= "SELECT * FROM lop";
	$result = dbQuery($sql);
	$count 	= dbNumRows($result);
	...
	$pageno = '<ul class="pagination pagination-sm no-margin pull-right">';
	for($i=1; $i<=$pages; $i++)	{
		$pageno .= "<li><a href=\"?v=CLASS&page=$i\">".$i."</a></li>";
	}
	$pageno .= 	"</ul>";
	return $pageno;
}
...
function getlop() {
	...
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$records[] = array("cid" => $id,"cidmalop" =>$idMaLop, "ctenlop" => $tenLop, "csiso"=>$siSo);	
	}//while
	return $records;
}
function getlopid() {
...
function getfulllop() {
	...
	$sql 	= "SELECT DISTINCT Subject FROM tbl_thoikhoabieu";
	//echo $sql;
	$result = dbQuery($sql);
	$records = array();
	while($row = dbFetchAssoc($result)) {
...
function gettengv() {
	...
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$records[] = array("tfullName"=>$fullName);	
	}//while
	return $records;
}
