...
function deleteClass() {
	$clId	= $_GET['cid'];
	$dsql	= "DELETE FROM lop WHERE id = $clId";
	dbQuery($dsql);
	header('Location: ../views/?v=CLASS&msg=' . urlencode('Class record successfully deleted.'));
	exit();
}
function deleteHocKy() {
	$kId	= $_GET['kmahocky'];
	$dsql	= "DELETE FROM hocky WHERE maHocKy = $kId";
	dbQuery($dsql);
	header('Location: ../views/?v=HOCKY&msg=' . urlencode('Hoc Ky record successfully deleted.'));
	exit();
}
